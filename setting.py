...
class TranslationSetting(QDialog):
    ...
    def layout_engine(self):
        ...
        self.genai_port = QSpinBox()
        self.genai_port.setRange(1, 65535)
        genai_layout.addRow(_('Port'), self.genai_port)
        self.genai_context = QSpinBox()
        self.genai_context.setRange(0, 50)
        genai_layout.addRow(_('Context Limit'), self.genai_context)
        ...
        self.disable_wheel_event(self.genai_port)
        self.disable_wheel_event(self.genai_context)
        ...
        def manual_fetch_ai_models():
            if self.api_keys.toPlainText().strip() != '' or \
                    issubclass(self.current_engine, OllamaTranslate):
                fetch_ai_models()
            else:
        ...
        def auto_fetch_ai_models():
            if issubclass(self.current_engine, GenAI) \
                    ...
                    and (self.api_keys.toPlainText().strip() != '' or
                         issubclass(self.current_engine, OllamaTranslate)):
                fetch_ai_models()
            else:
                init_ai_models()
        ...
        def show_genai_preferences(config):
            ...
            self.genai_endpoint.setVisible(not is_ollama)
            genai_layout.labelForField(self.genai_endpoint).setVisible(
                not is_ollama)

            # Port
            self.genai_port.setVisible(is_ollama)
            genai_layout.labelForField(self.genai_port).setVisible(is_ollama)
            if is_ollama:
                try:
                    self.genai_port.valueChanged.disconnect()
                except TypeError:
                    pass
                self.genai_port.setValue(
                    config.get('port', self.current_engine.port))
                self.genai_port.valueChanged.connect(
                    lambda value: config.update(port=value))

            # Context Limit
            self.genai_context.setVisible(is_ollama)
            genai_layout.labelForField(self.genai_context).setVisible(
                is_ollama)
            if is_ollama:
                try:
                    self.genai_context.valueChanged.disconnect()
                except TypeError:
                    pass
                self.genai_context.setValue(
                    config.get('context_limit',
                               self.current_engine.context_limit))
                self.genai_context.valueChanged.connect(
                    lambda value: config.update(context_limit=value))
